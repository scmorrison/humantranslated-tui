...
layout.storyEditSubmit.on('press', function() {
  //CheckIn
  humantranslated.setSeen(conf.APIKEY, {
...
layout.listTitle.on('keypress', function(ch, key) {
  if(key.name === "down" || key.name === "j") {
    ...
    layout.tabStories.focus();
      ...
      //If we have content 
      if(result.length > 0) {
        layout.storyView.setContent("{yellow-fg}"+result[arrayIndex].title+"{/yellow-fg}\n\n"+result[arrayIndex].content);
    ...
    //If we have content
    if(result.length > 0) {
      layout.storyEditForm.setContent("{center}{yellow-fg}"+result[arrayIndex].title+"{/yellow-fg}{/center}\n"+result[arrayIndex].content);
      layout.screen.append(layout.storyEditForm);
      layout.storyEditSubmit.focus();
    }
  }
  layout.screen.render();
});
